...
const ImageUploader = ({ uploading }) => {
    ...
    const handleFileChange = (event) => {
        ...
        if (file) {
            ...
            dispatch({ type: 'CHANGE_IMG', payload: null });
        }
    };
    ...
    const handleSaveImg = (event) => {
        ...
        dispatch({ type: 'CHANGE_IMG', payload: imagePreviewUrl });
    };
        ...
        <>
            <div className="p-4 border rounded-2xl max-w-[425px] min-h-[250px] max-h-[350px] w-full border-grey-border mt-1.5">
                <div className="flex flex-col border border-grey-border h-full object-cover py-4 px-4 rounded-2xl items-center justify-center bg-grey-border">
                    {imagePreviewUrl ? (
                        <img src={imagePreviewUrl} alt="Preview" className="rounded-2xl w-full h-full object-cover" />
                    ) : (
                        <FotoIcon />
                    )}
                </div>
            ...
            <button htmlFor="file-upload" onClick={handleSaveImg} className="bg-grey-900 text-white font-medium text-sm px-4 py-3 sm:w-[126px] mt-4 rounded-lg hover:bg-white hover:text-grey-900 box-border border hover:border-grey-900 duration-300 transition-all">Save Image</button>
        </>
    );
};
